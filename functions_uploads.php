...
function get_uploads($conversation_id){
    global $conn;
    ...
    if ($conversation_id !== null) {
        ...
        try {
            ...
            $uploads_query->bindParam(':conversation_id', $conversation_id);
...
function create_upload($upload) {
    global $conn;
    if ( !empty($upload->conversation_id)  && !empty($upload->file_contents)  ){
        ...
        try {
            ...
            $query = "INSERT INTO uploads (conversation_id, message_id, filename, extension) VALUES (:conversation_id, :message_id, :filename, :extension)";
            $upload_query = $conn->prepare($query);
            $upload_query->bindParam(':conversation_id', $upload->conversation_id);
    ...
    } else { // upload conversation_id was blank
        return false;
    }
}
...
function processUpload($upload) {
    ...
    $upload->conversation_id =  intval($upload->conversation_id);
    $upload->message_id =  intval($upload->message_id);
